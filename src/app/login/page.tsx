...
const Login = () => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (!response?.ok) {
                toast.error(response?.error as string ?? "Error while signing-in")
            } else {
        ...
        <div className='min-h-full w-full grid place-content-center'>
            <form onSubmit={handleSubmit} className='my-5 card'>
                <fieldset className="fieldset bg-base-200 border-base-300 rounded-box w-xs border p-4 card-body">
                    <h2 className="card-title mx-auto">Login to Stream.ly</h2>
                    ...
                    <label className="input input-primary">
                        ...
                        <input type="text" placeholder="Email or Username" required value={identifier} onChange={(e) => setIdentifier(e.target.value)} />
                    </label>
                    ...
                    <button className="btn btn-outline btn-primary mt-2 w-fit mx-auto" type="submit" disabled={loading || googleLoading || githubLoading || discordLoading}>
                        {
                    ...
                    <button className="btn bg-black text-white border-black" disabled={loading || googleLoading || githubLoading || discordLoading} type='button' onClick={handleGithubSign}>
                        ...
                        {
                            githubLoading ? (<span className="loading loading-dots loading-xs"></span>) : ("Login with Github")
                        }
                    </button>
                    ...
                    <p className='text-center'>Don't have an account? <span className="link link-primary cursor-pointer" onClick={() => router.push('/register')}>Register</span></p>
                </fieldset>
            </form>
        </div>
    )
